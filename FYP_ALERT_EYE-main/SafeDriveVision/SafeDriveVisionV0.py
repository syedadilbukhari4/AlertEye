...
def calculate_head_angle(eye_left, eye_right, nose_tip):
    ...
    return angle_deg
# Charger le modèle 
weights_path = 'F:\SafeDriveVision\SafeDriveVision\weights\yolov5m.pt'  # Mettez à jour avec le chemin exact du fichier de poids
...
while True:
    ...
    for detection in detections:
        if int(detection[5]) == 67:  # 67 is the index of 'cell phone'
            ...
            conf = float(detection[4])  # confidence score (0.0 - 1.0)
    
            if conf > 0.4:  # only proceed if confidence > 50%
                cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
                cv2.putText(img, f'Cell Phone {conf:.2f}', (x1, y1 - 10),
                ...
                if COUNTER2 >= 3:
                    cv2.putText(img, "Rangez votre CELL PHONE!", (x1, y1 - 30),
